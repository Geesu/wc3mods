...
public _MYSQLX_SetData( failstate, Handle:query, error[], errnum, data[], size )
{
	...
	else
	{
		...
		if ( SQL_MoreResults( query ) )
		{
			...
			p_data[id][P_SKILL1]	= SQL_ReadResult( query, 1 );
			p_data[id][P_SKILL2]	= SQL_ReadResult( query, 2 );
			p_data[id][P_SKILL3]	= SQL_ReadResult( query, 3 );
			p_data[id][P_ULTIMATE]	= SQL_ReadResult( query, 4 );
		}
