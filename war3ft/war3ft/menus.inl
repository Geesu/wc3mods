...
public MENU_BuildAll()
{

	/*****  SHOPMENU CREATION *****/
	
	new szItemName[64], szTmp[128], iCurMenu = MENU_SHOPMENU1;

	g_Menu[MENU_SHOPMENU1] = menu_create( "Shopmenu 1\R$  Cost", "_MENU_Shopmenu1" );
	g_Menu[MENU_SHOPMENU2] = menu_create( "Shopmenu 2\R$  Cost", "_MENU_Shopmenu2" );

	
	// Lets add the items to the menus
	for ( new i = 0; i < MAX_SHOPMENU_ITEMS; i++ )
	{
		// Now the following items belong in shopmenu 2
		if ( i == MAX_ITEMS_PAGE )
		{
			iCurMenu = MENU_SHOPMENU2;
		}

		LANG_GetItemName( i, LANG_SERVER, szItemName, 63 );

		// These items don't exist in DOD
		if ( ( i == ITEM_SCROLL ) )
		{
			formatex( szTmp, 127, "\d %s\y\R%d", szItemName, ITEM_COST[i] );

			menu_additem( g_Menu[iCurMenu], szTmp, "ZOMG!", 1 );
		}

		// Everything else is allowed!
		else
		{
			formatex( szTmp, 127, "\w %s\y\R%d", szItemName, ITEM_COST[i] );

			menu_additem( g_Menu[iCurMenu], szTmp, "" );
		}

	}

	menu_setprop(g_Menu[MENU_SHOPMENU1], MPROP_PERPAGE, 0);
	menu_setprop(g_Menu[MENU_SHOPMENU2], MPROP_PERPAGE, 0);
}
